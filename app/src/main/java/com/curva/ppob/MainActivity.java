...
public class MainActivity extends Activity {
        ...
        new Thread(new Runnable() {
				@Override public void run() {
					try {
						...
						conn.setUseCaches(false); conn.setRequestProperty("User-Agent", "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36"); conn.setRequestProperty("Cache-Control", "no-cache"); conn.setRequestProperty("Pragma", "no-cache"); conn.setRequestProperty("Accept", "application/json");
    ...
    private boolean isMaintenanceTime() {
        ...
        if (hour == 23 && minute >= 30) { return true; } else if (hour == 0 && minute <= 10) { return true; } return false;
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getExtras() != null) {
            ...
            if (!newUrl.equals(HOME_URL)) { if (isMaintenanceTime()) { showMaintenanceScreen(webView); } else if (isNetworkAvailable()) { webView.loadUrl(newUrl); } else { showOfflineScreen(webView); } }
        }
    }
    ...
    private void autoPrintReceipt(final String textToPrint, final String logoBase64) {
        ...
        new Thread(new Runnable() {
                @Override public void run() {
                    boolean isPrinted = false; String printerName = ""; UUID uuid = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
                    byte[] imageBytes = null;
                    if (logoBase64 != null && logoBase64.startsWith("data:image")) {
                        try {
                            String pureBase64 = logoBase64.substring(logoBase64.indexOf(",") + 1); byte[] decodedString = Base64.decode(pureBase64, Base64.DEFAULT); Bitmap bitmap = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
                            if(bitmap != null){
                                int targetWidth = 360; targetWidth = (targetWidth / 8) * 8; 
                                int targetHeight = (int) (bitmap.getHeight() * ((float) targetWidth / bitmap.getWidth()));
                    ...
                    for (BluetoothDevice device : devicesToTry) {
                        try {
                            BluetoothSocket socket = device.createRfcommSocketToServiceRecord(uuid); socket.connect(); OutputStream outputStream = socket.getOutputStream();
                            outputStream.write(new byte[]{27, 64}); 
                            if (imageBytes != null) {
                                outputStream.write(new byte[]{27, 97, 1}); outputStream.flush(); int chunkSize = 256; 
                                for (int i = 0; i < imageBytes.length; i += chunkSize) { int length = Math.min(chunkSize, imageBytes.length - i); outputStream.write(imageBytes, i, length); outputStream.flush(); Thread.sleep(15); }
                                outputStream.write("\n\n".getBytes()); outputStream.flush(); Thread.sleep(400); 
                            }
                            outputStream.write(textToPrint.getBytes("UTF-8")); outputStream.write("\n\n\n".getBytes()); outputStream.flush(); socket.close();
                            isPrinted = true; printerName = device.getName(); break; 
    ...
    private byte[] decodeBitmapToEscPos(Bitmap bmp) {
        int width = bmp.getWidth(); int height = bmp.getHeight(); int xBytes = (width + 7) / 8;
        byte[] data = new byte[8 + (xBytes * height)];
        data[0] = 0x1D; data[1] = 0x76; data[2] = 0x30; data[3] = 0x00; data[4] = (byte) (xBytes % 256); data[5] = (byte) (xBytes / 256); data[6] = (byte) (height % 256); data[7] = (byte) (height / 256);
        int idx = 8;
    ...
    private void shareImage(String base64Data) {
        try {
            byte[] decodedString = Base64.decode(base64Data, Base64.DEFAULT); Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
            File cachePath = new File(getCacheDir(), "images"); cachePath.mkdirs(); 
            File file = new File(cachePath, "struk_curva_payment.png"); FileOutputStream stream = new FileOutputStream(file); decodedByte.compress(Bitmap.CompressFormat.PNG, 100, stream); stream.close();
            Uri contentUri = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", file);
            if (contentUri != null) {
                Intent shareIntent = new Intent(); shareIntent.setAction(Intent.ACTION_SEND); shareIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                shareIntent.setDataAndType(contentUri, getContentResolver().getType(contentUri)); shareIntent.putExtra(Intent.EXTRA_STREAM, contentUri); shareIntent.putExtra(Intent.EXTRA_TEXT, "Berikut adalah bukti pembayaran dari Curva Payment."); 
    ...
    private void shareReceiptText(String base64Data, String customCaption) {
        try {
            byte[] decodedString = Base64.decode(base64Data, Base64.DEFAULT); Bitmap decodedByte = BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
            File cachePath = new File(getCacheDir(), "images"); cachePath.mkdirs(); 
            File file = new File(cachePath, "struk_pembayaran.png"); FileOutputStream stream = new FileOutputStream(file); decodedByte.compress(Bitmap.CompressFormat.PNG, 100, stream); stream.close();
            Uri contentUri = FileProvider.getUriForFile(this, getPackageName() + ".fileprovider", file);
            if (contentUri != null) {
                Intent shareIntent = new Intent(); shareIntent.setAction(Intent.ACTION_SEND); shareIntent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
                shareIntent.setDataAndType(contentUri, getContentResolver().getType(contentUri)); shareIntent.putExtra(Intent.EXTRA_STREAM, contentUri); shareIntent.putExtra(Intent.EXTRA_TEXT, customCaption); 
                startActivity(Intent.createChooser(shareIntent, "Bagikan Struk Melalui..."));
            }
    ...
    public void saveImage(String base64String, String fileName) {
        try {
            byte[] decodedString = android.util.Base64.decode(base64String, android.util.Base64.DEFAULT); android.graphics.Bitmap bitmap = android.graphics.BitmapFactory.decodeByteArray(decodedString, 0, decodedString.length);
            android.content.ContentValues values = new android.content.ContentValues(); values.put(android.provider.MediaStore.Images.Media.DISPLAY_NAME, fileName); values.put(android.provider.MediaStore.Images.Media.MIME_TYPE, "image/png");
            if (android.os.Build.VERSION.SDK_INT >= 29) { values.put(android.provider.MediaStore.Images.Media.IS_PENDING, 1); }
            android.content.ContentResolver resolver = MainActivity.this.getContentResolver(); android.net.Uri uri = resolver.insert(android.provider.MediaStore.Images.Media.EXTERNAL_CONTENT_URI, values);
            if (uri != null) {
                java.io.OutputStream out = resolver.openOutputStream(uri); bitmap.compress(android.graphics.Bitmap.CompressFormat.PNG, 100, out); out.flush(); out.close();
                if (android.os.Build.VERSION.SDK_INT >= 29) { values.clear(); values.put(android.provider.MediaStore.Images.Media.IS_PENDING, 0); resolver.update(uri, values, null, null); }
                MainActivity.this.runOnUiThread(new Runnable() { public void run() { android.widget.Toast.makeText(MainActivity.this, "Gambar berhasil disimpan ke Galeri", android.widget.Toast.LENGTH_SHORT).show(); } });
            } else { throw new Exception("URI is null"); }
    ...
    private String getContactsFromDevice() {
        ...
        try {
            ...
            if (cursor != null && cursor.getCount() > 0) {
                int nameIndex = cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.DISPLAY_NAME); int phoneIndex = cursor.getColumnIndex(ContactsContract.CommonDataKinds.Phone.NUMBER);
                while (cursor.moveToNext()) {
                    String name = cursor.getString(nameIndex); String phone = cursor.getString(phoneIndex);
                    if (phone != null) { phone = phone.replaceAll("[^0-9]", ""); if (phone.startsWith("62")) { phone = "0" + phone.substring(2); } if (phone.length() > 13) { phone = phone.substring(phone.length() - 13); } }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == REQUEST_CONTACT_PERMISSION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) { openContactPicker(); } else { webView.evaluateJavascript("javascript:onAndroidContactsError('Izin ditolak')", null); }
        } 
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == PICK_CONTACT_REQUEST && resultCode == RESULT_OK && data != null) {
            Uri contactUri = data.getData(); String contactNumber = "";
            try { Cursor phoneCursor = getContentResolver().query(contactUri, new String[]{ContactsContract.CommonDataKinds.Phone.NUMBER}, null, null, null); if (phoneCursor != null && phoneCursor.moveToFirst()) { contactNumber = phoneCursor.getString(0); phoneCursor.close(); } } catch (Exception e) { e.printStackTrace(); }
            if (contactNumber != null && !contactNumber.isEmpty()) { contactNumber = contactNumber.replaceAll("[^0-9]", ""); if (contactNumber.startsWith("62")) { contactNumber = "0" + contactNumber.substring(2); } if (contactNumber.length() > 13) { contactNumber = contactNumber.substring(contactNumber.length() - 13); } }
            final String phoneNumber = contactNumber;
            webView.post(new Runnable() {
                    @Override public void run() { webView.evaluateJavascript("javascript:try{onContactPicked('" + phoneNumber + "');}catch(e){}", null); webView.evaluateJavascript("javascript:(function(){var input=document.getElementById('target');if(input){input.value='" + phoneNumber + "';var ev=new Event('input');input.dispatchEvent(ev);}})()", null); webView.evaluateJavascript("javascript:try{closeContactModal();}catch(e){}", null); }
                });
        }
        ...
        try {
            com.google.firebase.messaging.FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(new com.google.android.gms.tasks.OnCompleteListener<String>() {
                    @Override public void onComplete(com.google.android.gms.tasks.Task<String> task) { if (!task.isSuccessful()) { fcmToken = ""; return; } fcmToken = task.getResult(); sendFcmTokenToWeb(); }
                });
